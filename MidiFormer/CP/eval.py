...
def main():
    ...
    if args.task == 'melody' or args.task == 'velocity':
        dataset = 'pop909'
        model = TokenClassification(midi_former, args.class_num, args.hs)
        seq_class = False
    elif args.task == 'composer' or args.task == 'emotion':
        dataset = args.task
        model = SequenceClassification(midi_former, args.class_num, args.hs)
        seq_class = True
...
# python3 eval.py --task=melody --pos_type absolute --use_fif --ckpt=result/finetune/melody_cnn-a/model_best.ckpt
# python3 eval.py --task=velocity --pos_type absolute --use_fif --ckpt=result/finetune/velocity_cnn-a/model_best.ckpt
# python3 eval.py --task=emotion --pos_type absolute --use_fif --ckpt=result/finetune/emotion_cnn-a/model_best.ckpt
# python3 eval.py --task=composer --pos_type absolute --use_fif --ckpt=result/finetune/composer_cnn-a/model_best.ckpt