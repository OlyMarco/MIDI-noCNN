...
class FinetuneTrainer:
    def __init__(self, midi_former, train_dataloader, valid_dataloader, test_dataloader, layer,
        ...
        # ckpt=torch.load("result/pretrain/cnn-10/model_best_0.7858.ckpt", map_location=self.device)
        # self.model.load_state_dict(ckpt['state_dict'])
        # print("冻结MidiBERT参数，只训练分类头")
        # for name, param in self.model.named_parameters():
        #     if 'midi_former' in name:  # 冻结所有MidiBERT相关参数
        #         param.requires_grad = False
        print("parameter num:", sum(p.numel() for p in self.model.parameters() if p.requires_grad))
...
#            print(name, param.requires_grad)
    ...
    def iteration(self, training_data, mode, seq):
        ...
        for batch_data in pbar:  # (batch, 512, 768)
            if isinstance(batch_data, dict):
                x = batch_data['segments'].to(self.device)  # (batch, seq_len, 4)
                pctm_batch = batch_data['pctm'].to(self.device)  # (batch, 12, 12)
                nltm_batch = batch_data['nltm'].to(self.device)  # (batch, 12, 12)
                y = batch_data['labels'].to(self.device)  # (batch, seq_len)
            ...
            y_hat = self.model.forward(x, attn, self.layer, pctm=pctm_batch, nltm=nltm_batch)     # seq: (batch, class_num) / token: (batch, 512, class_num)
