...
class TokenClassification(nn.Module):
    ...
    def forward(self, y, attn, layer, pctm=None, nltm=None):
        # feed to former
        y = self.midi_former(y, attn, output_hidden_states=True, mode=self.mode, pctm=pctm, nltm=nltm)
...
class SequenceClassification(nn.Module):
    ...
    def forward(self, x, attn, layer, pctm=None, nltm=None):             # x: (batch, 512, 4)
        x = self.midi_former(x, attn, output_hidden_states=True, pctm=pctm, nltm=nltm)   # (batch, 512, 768)
        #y = y.last_hidden_state         # (batch_size, seq_len, 768)
